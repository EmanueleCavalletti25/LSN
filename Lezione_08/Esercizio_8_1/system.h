...
const double _delta = 10; //parameter giving the jump to new position
...
protected:
    ...
    double pos_old = 0.;
    double _temp = 100.;
    double _energy_old = 200.;
    
    

public:
    Random RND;
    unique_ptr<Waves_function> WF_test = unique_ptr<Wave_Variational_test>(new Wave_Variational_test()); //Pointer to wave function class
    unique_ptr<Waves_function> Energy_loc = unique_ptr<Energy_Variational_test>(new Energy_Variational_test()); //Pointer to Energy variational measure class
