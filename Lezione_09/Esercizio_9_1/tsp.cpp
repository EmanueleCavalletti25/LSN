...
// FUNCTION TO INITIALIZE THE PROBLEM
    void tsp :: initialize(){     //function to initialize the system: shape and other parameters
      ...
      for(int i=0; i<_ncity; i++){ 
        ...
        if (_shape == 0){_cityes(i).set_pos(_rnd.Circonference((double)_r));};
        if (_shape == 1){_cityes(i).set_pos(_rnd.Square((double)_r));}
      }
  ...
  return;
    ...
    void tsp :: initialize_prop(){
    
  ofstream out_m;
  ofstream out_p;
  if (_shape == 0){out_m.open("./OUTPUT/Loss_media_circunference_best.dat");}
  if (_shape == 1){out_m.open("./OUTPUT/Loss_media_square_best.dat");}
  if (_shape == 0){out_p.open("./OUTPUT/Loss_media_circunference_best_single.dat");}
  if (_shape == 1){out_p.open("./OUTPUT/Loss_media_square_best_single.dat");}

  out_m << "BEST LOSS FUNCTION MEAN and DV_STD"<< endl;
  out_p << "BEST LOSS FUNCTION SINGLE "<< endl;
...
void tsp:: mutation (vec& v){
    ...
    double tsp::write_loss_function(){
      ...
      ofstream out_m;
      if (_shape == 0){out_m.open("./OUTPUT/Loss_media_circunference_best.dat", ios::app);}
      if (_shape == 1){out_m.open("./OUTPUT/Loss_media_square_best.dat", ios::app);}
    ...
    void tsp:: write_loss_function_single(){
      vec loss_vectore = Loss_vector(m_pop);

      vec v = arma::sort(loss_vectore);
      ofstream out_m;
      if (_shape == 0){out_m.open("./OUTPUT/Loss_media_circunference_best_single.dat", ios::app);}
      if (_shape == 1){out_m.open("./OUTPUT/Loss_media_square_best_single.dat", ios::app);}
    ...
    void tsp::write_best_path(mat pop){
      vec loss_vec = Loss_vector(m_pop); 
      ofstream out_mat;
      
      if (_shape == 0){out_mat.open("./OUTPUT/best_path_circunference.dat");}
      if (_shape == 1){out_mat.open("./OUTPUT/best_path_square.dat");}
    ...
    double tsp :: Loss_best_media(vec v){
      ...
      v = arma::sort(v); //nella prima metà, ne faccio la media
      double media = 0.;
      double counter = 0.;
    ...
    void tsp::set_row_pop(vec new_row,int idx_row){ //function to modify the row of matrix population
      ...
      vec(m_pop.row(idx_row).t()) = new_row;
    };
