...
// FUNCTION TO INITIALIZE THE PROBLEM
    void tsp :: initialize(int rank){     //function to initialize the system: shape and other parameters
        ...
        string filename_input_nomi;
        
        filename_input = "./INPUT/cap_prov_ita.dat";
        filename_input_nomi = "./INPUT/prov_ita.txt";
        
        //BEGIN LECTURE
        ifstream inputFile(filename_input);
        ifstream inputFile_nomi(filename_input_nomi);
        if (!inputFile.is_open()){
          std::cerr << "Errore: impossibile aprire il file " << filename_input <<  endl;
          exit(-1);  // Esce dal programma con codice di errore
        }
        if (!inputFile_nomi.is_open()){
          std::cerr << "Errore: impossibile aprire il file " << filename_input <<  endl;
          exit(-1);  // Esce dal programma con codice di errore
        }
        
        
        int counter_dat = 0;          //see that # name and coordinate concide each other.
        int counter_name = 0;
        string linea;
        string linea_n;
        ...
        while (getline(inputFile_nomi, linea_n)) {
            ++counter_name;
        }

        MPI_Barrier(MPI_COMM_WORLD);
        if (counter_dat!=counter_name){
          cerr << "N_CITY= " << counter_dat << " NAME"<< counter_name << endl;
          cerr << "ERROR! NUMBER OF NAME AND CITY COORDINATE DON'T CONCIDE EACH OTHER:" << endl;
          exit (-1);
        }
        ...
        inputFile_nomi.clear();
        inputFile_nomi.seekg(0);
        inputFile_nomi.close();

        _ncity = counter_dat;

        ifstream input_data(filename_input); // Start reading ./INPUT/prov_ita.dat
        ifstream input_name(filename_input_nomi); // Start reading ./INPUT/nomi_.txt
        ofstream coutf;
        string output_f = "./OUTPUT/output.dat";
        coutf.open(output_f);

        coutf << "SO LONG JOURNEY IN ITALY: " << endl;
        ...
        for (int i = 0; i <_ncity ; i++){
           ...
           _cityes(i).set_pos(pos_c);
           _cityes(i).set_name(city_name);
           MPI_Barrier(MPI_COMM_WORLD);
        }
        ...
        //READING POPULATION:
        ifstream input;
        input.open("./INPUT/input.dat");
        if (!input.is_open()){
          std::cerr << "Errore: impossibile aprire il file input.dat " << endl;
          exit(-1);  // Esce dal programma con codice di errore
        }
        string property;
        ...
        if (property == "POPULATION") {
            input >> _npop;
            if(rank==0){cout << "POPULATION = " << _npop << endl;}
            m_pop = arma::zeros(_npop, _ncity - 1);  // Matrix della popolazione: n_city - 1 per mescolamenti migliori
        } else if (property == "ENDINPUT") {
            if(rank==0){cout << "Reading input completed!" << endl;}
            break;
        }
      }

        if (rank == 0){cout <<"POPULATION: " << _npop << " CITY " << _ncity << endl;}
        input.close();
  ...
  void tsp :: initialize_prop(int rank){
        ...
        ofstream out_m;
        out_m.open("./OUTPUT/Loss_media_italy_best_"+std::to_string(rank)+"_.dat");
        out_m << "BEST ITALY LOSS FUNCTION"<< endl;
        out_m.close();
     ...
     //if (rank == 0){m_pop.print("I BORGHI PIù BELLI DI ITALIA");}
...
void tsp:: mutation (vec& v){
    ...
    //FUNCTION TO WRITE OUTPUT
      ...
      ofstream out_m;
      out_m.open("./OUTPUT/Loss_media_italy_best_"+std::to_string(rank)+"_.dat", ios::app);
      out_m << media_50 << endl;
      out_m.close();
    ...
    void tsp::write_best_path(mat pop,int rank){
      ...
      ofstream out_mat;
      out_mat.open("./OUTPUT/Path_italy_best_"+std::to_string(rank)+"_.dat");
      ...
      void tsp:: write_core_best(int rank,int min_rank, double global_min){
          if (rank==0){
          ofstream out_best;
          out_best.open("./OUTPUT/output_italy_best_.dat");
