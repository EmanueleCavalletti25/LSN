...
vec Random :: Circonference (double radius){
   vec point_on_circonference(2,fill::zeros);
   double theta = Rannyu(0.,2.*M_PI);
   point_on_circonference [0] = radius*cos(theta);
   point_on_circonference [1] = radius*sin(theta);
   return point_on_circonference;
   
}

vec Random :: Square (double lato){
   vec point_in_square(2,fill::zeros);
   point_in_square [0] = (double)lato*Rannyu(-1.,1.);
   point_in_square [1] = (double)lato*Rannyu(-1.,1.);
