...
private:
  ...
  int _n_opt_temp;        // Number of optimize temperature found
  ...
  double _best_diff = 0.; //variable utilized to compare differences between values
  double _T_attesa;        // set the desired temperature depending on the phase
  string _phase;              //set type of the system: phase (SOLID, LIQUID, GAS) or ISING
  string _SMT;               //set type of the simulation choose: phase NVE or NVT
  string _task;
  string _method;
  double _T_average;
  double _T_precision = 0.03;
  double _fr_accept;
  ...
  vec _optimize_temp;    // Measured the optimized temperature

public: // Function declarations
  int get_nbl();              // Get the number of blocks
  int get_nsteps();           // Get the number of steps in each block
  void initialize( double, Mode);          // Initialize system properties
  void initialize_properties(double counter,Mode);// Initialize properties for measurement
  void finalize(double counter, Mode );            // Finalize system and clean up
  ...
  void block_reset(int blk, Mode mode, double counter);  // Reset block averages
  void measure();             // Measure properties of the system
  void averages(int blk, Mode);     // Compute averages of properties
  ...
  double Gibbs (int i); //Calculate the conditional probability for flipping spin in Ising model.
  
  
  bool equilibration_fine (double att_Temp, double counter); //compare last progressive average block temperature with the ideal one and decide if restart the program.
  bool equilibration_rough(double att_Temp, double counter ); //rough equilibration in order to understand around which temperature the system stabilized
  void modify_temp_input(const string& filename, double T_STAR); //function to modify TEMP input.dat
  void modify_delta_input(const string& filename, double new_delta); //function to modify DELTA input.dat
  void set_best_diff(); //function to set _best_diff_ variable.
  string get_phase(); //function to get the type of the system
  string get_SMT();   //function to get the type of the simulation, between NVE or NVT   
  void set_phase_of_system(string phase);
  void set_SMT_of_system();
  double _set_T_attended(string phase);
  double get_temp();
  void set_task(string task, string method);
