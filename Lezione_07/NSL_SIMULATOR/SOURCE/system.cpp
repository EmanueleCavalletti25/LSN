...
double System :: Gibbs(int i){
  ...
  //da modificare in presenza di un campo H
   return prob_cond;
}
...
void System :: initialize( double counter, Mode mode){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream Primes("../INPUT/"+_method+"/"+_phase+"/Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  int seed[4]; // Read the seed of the RNG
  ifstream Seed("../INPUT/"+_method+"/"+_phase+"/seed.in");
  ...
  switch (mode){
    case MODE1:
      filename_input = "../INPUT/"+_method+"/"+_phase+"/input_equilibration.dat";
      filename_outputa = "../OUTPUT/"+_SMT+"/"+_phase+"/EQUILIBRATION/acceptance.dat";
      
    break;
    case MODE2:
      filename_input = "../INPUT/"+_method+"/"+_phase+"/input.dat";
      filename_outputa = "../OUTPUT/"+_SMT+"/"+_phase+"/acceptance.dat";
    break;
    default:
      break;
    }
  ...
  if( property == "SIMULATION_TYPE" ){
      input >> _sim_type;
      set_SMT_of_system();
      input.clear();         // Resetta lo stato dello stream (ad esempio dopo eof o errori)
      input.seekg(0);        // Torna all'inizio del file
  ...
  //Switch in order to divide equilibration case and evolution case: divide output_file in a direct for equilibration
    switch (mode){
      ...
      output_f = "../OUTPUT/"+_SMT+ "/"+ _phase +"/output_" + formatted_counter + ".dat";
  ...
  while ( !input.eof() ){
    ...
    //switch: equilibration FOR MD, increase or decrease temperature of a fix factor, for searching adapting temperature
    //        evolution: chose a temperature or a set of temperature evolved them.
      switch (mode){
        case MODE1:
        input >> _temp;
        if (_sim_type == 0){
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open("../INPUT/"+_method+"/"+_phase +"/CONFIG/velocities.in");
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
...
void System :: initialize_properties(double counter, Mode mode){ // Initialize data members used for measurement of properties
  ...
  switch (mode){
    case MODE1:{
    input.open("../INPUT/"+_method+"/"+ _phase +"/properties_equilibration.dat");
    break;
    }
    case MODE2:{
    input.open("../INPUT/"+_method+"/"+ _phase +"/properties.dat");
    break;
    }
    default :{
  ...
  while ( !input.eof() ){
      input >> property;
          if( property == "POTENTIAL_ENERGY" ){
              ofstream coutp;
              if (_task == "autocorrelation"){coutp.open("../OUTPUT/"+_SMT+ "/AUTOCORRELATION/"+ _phase +"/potential_energy.dat");
              }else {coutp.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/potential_energy.dat");}
        ...
        ofstream coutk("../OUTPUT/"+_SMT+ "/"+ _phase +"/kinetic_energy.dat");
        ...
        index_property++;
      } else if( property == "TOTAL_ENERGY"  && _H == 0.){
        ofstream coutt;
        double t_temp = _temp; 
        ostringstream oss;
        oss << fixed << setprecision(2) << t_temp;
        switch (mode){
        case MODE1:
          if (_phase == "ISING"){
            if (_sim_type == 2){
            coutt.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_total_energy.dat");
            }else{coutt.open("../OUTPUT/"+_SMT+ "/" +_phase +"/EQUILIBRATION/GIBBS_total_energy.dat");}
          }else{coutt.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/total_energy.dat");}
          break;
        case MODE2:
          if (_phase == "ISING"){
          if (_sim_type == 2 ){coutt.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_total_energy_" + oss.str() + "_.dat");}
          if (_sim_type == 3 ){coutt.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_total_energy_" + oss.str() + "_.dat");}
        break;
          }else{coutt.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/total_energy.dat");}
        break;
        default:
              cout << "Error in open file total energy .dat ";
          break;
        }
        ...
        case MODE1:{
          if(_phase == "ISING"){
              coutte.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/temperature_" + formatted_temp + ".dat");
          }else{coutte.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/temperature_" + formatted_temp + ".dat");}
          break;
        }
        case MODE2:{
          coutte.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/temperature_" + formatted_temp + ".dat");
        ...
        ofstream coutpr("../OUTPUT/"+_SMT+ "/"+ _phase +"/pressure.dat");
        ...
        ofstream coutgr("../OUTPUT/"+_SMT+ "/"+ _phase +"/gofr.dat");
        ...
        ofstream coutmag;
        double t_temp = _temp; 
        ostringstream oss, oss_m;
        oss << fixed << setprecision(2) << t_temp;
        oss_m << fixed << setprecision(2) << _H;
        switch (mode){
        case MODE1:
          if (_sim_type == 2 ){coutmag.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_magnet_"+oss_m.str()+"_.dat");}
          if (_sim_type == 3 ){coutmag.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_magnet_"+oss_m.str()+"_.dat");}
          break;
        
        case MODE2:
          if (_sim_type == 2 ){coutmag.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_magnet_" + oss.str() + "_"+oss_m.str()+"_.dat");}
          if (_sim_type == 3 ){if(_H == 0){coutmag.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_magnet_" + oss.str()+"_.dat");
          }else{coutmag.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_magnet_" + oss.str() + "_"+oss_m.str()+"_.dat");}
        ...
        break;
        default:
              cout << "Error in open file magnet.dat ";
          break;
        }
        coutmag << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutmag.close();
        _nprop++;
        if (_H != 0.){
        ...
        index_property++;
      } else if( property == "SPECIFIC_HEAT" && _H == 0.){
        ofstream coutcv;
        double t_temp= _temp; 
        ostringstream oss;
        oss << fixed << setprecision(2) << t_temp;
        switch (mode){
        case MODE1:
          if (_sim_type == 2 ){coutcv.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_cv.dat");}
          if (_sim_type == 3 ){coutcv.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_cv.dat");}
          
          break;
        case MODE2:
          if (_sim_type == 2 ){coutcv.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_cv_" + oss.str() + "_.dat");}
          if (_sim_type == 3 ){coutcv.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_cv_" + oss.str() + "_.dat");}
        break;
        default:
              cout << "Error in open file cv.dat ";
          break;
        }
        ...
        index_property++;
      } else if( property == "SUSCEPTIBILITY" && _H == 0.){
        ofstream coutchi;
        double t_temp = _temp; 
        ostringstream oss;
        oss << fixed << setprecision(2) << t_temp;
        ...
        switch (mode){
        case MODE1:
          if (_sim_type == 2 ){coutchi.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_chi.dat");}
          if (_sim_type == 3 ){coutchi.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_chi.dat");}
          break;
        case MODE2:
          
          if (_sim_type == 2 ){coutchi.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_chi_" + oss.str() + "_.dat");}
          if (_sim_type == 3 ){coutchi.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_chi_" + oss.str() + "_.dat");}
        break;
        default:
              cout << "Error in open file chi.dat ";
          break;
        }
        ...
        string formatted_counter = oss.str();
          ...
          switch (mode){
        case MODE1:{
          coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/output_" + formatted_counter + ".dat",ios::app);
          break;
        }
        case MODE2:{
          
          coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/output_" + formatted_counter + ".dat",ios::app);
        break;
        }
        default:{
...
void System :: finalize(double counter, Mode mode){
  ...
  ofstream coutf;
    ...
    string output_f;
      switch (mode){
      case MODE1:
      output_f = "../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/output_" + formatted_counter + ".dat";
      break;
      case MODE2:
      output_f = "../OUTPUT/"+_SMT+ "/"+ _phase +"/output_" + formatted_counter + ".dat";
      break;
...
void System :: write_configuration(){
  ofstream coutf;
  ostringstream oss_m;
  oss_m << fixed << std::setprecision(2) << _H;
  
  if(_sim_type < 2){
    coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    if (_sim_type == 2 ){
      if (_H== 0.){
      coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/METROPOLIS_config.spin");
      }else{coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/METROPOLIS_config_"+oss_m.str()+"_.spin");}
      }
    if (_sim_type == 3 ){if (_H== 0.){
      coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/GIBBS_config.spin");
      }else{coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/GIBBS_config_"+oss_m.str()+"_.spin");}
    }
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ...
  oss_m << fixed << setprecision(2) << _H;
  if (_sim_type < 2){
  cinf.open("../INPUT/"+_method+"/"+ _phase +"/CONFIG/config.xyz");
  ...
  if (_restart == 0 and _sim_type >= 2){
      int spin = 1; // set a first configuration where spin are totally ordinated;
      for (int  i = 0; i<_npart; i++){
  ...
  if(_restart and _sim_type > 1){
    ...
    ifstream cinf;
    if (_sim_type == 2 ){
      if(_H==0.){cinf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/METROPOLIS_config.spin");
      }else{cinf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/METROPOLIS_config_"+oss_m.str()+"_.spin");}
      }
    if (_sim_type == 3 ){
      if(_H==0.){cinf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/GIBBS_config.spin");
      }else{cinf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/CONFIG/GIBBS_config_"+oss_m.str()+"_.spin");}
    }
    for(int i=0; i<_npart; i++){
      cinf >> spin;
      _particle(i).setspin(spin);
...
void System :: measure(){ // Measure properties
  ...
  double sum_spin = 0.0; // summing spin of the system
  double virial=0.0;
  ...
  if (_measure_temp and _measure_pressure) _measurement(_index_pressure) =  _rho*_measurement(_index_temp)+ (48.*pression_temp/(3.*(double)_npart)+_ptail)/_volume;



  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if (_measure_tenergy and _measure_cv) { _measurement(_index_cv) =  pow(_measurement(_index_tenergy)*_npart,2);}
                                                                 


  if (_measure_chi or _measure_magnet){
    for (int j= 0; j< _npart; j++){
      sum_spin += double (_particle(j).getspin());
    }
    
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if (_measure_magnet) _measurement(_index_magnet) = sum_spin/(double)_npart; 
  
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if (_measure_chi) _measurement(_index_chi) = _beta*pow(sum_spin,2)/(double)_npart; 
...
void System :: averages(int blk, Mode mode){
  ...
  /*for (int i = 0; i < _n_bins; i++){
    cout << "distance: " << _bin_size*(double)(i+1)<<" --> "<<_measurement(_index_gofr+i) << endl;
  }*/
  ...
  if (_measure_cv){
    ...
    _average(_index_cv) *= pow(_beta,2)/(double)_npart;
  }
  ...
  // TEMPERATURE /////////////////////////////////////////////////////////////////////
    if (_measure_temp){
      ...
      string formatted_temp = oss.str();
      coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/temperature_" + formatted_temp + ".dat",ios::app);
    ...
    // TOTAL ENERGY //////////////////////////////////////////////////////////////
      if (_measure_tenergy){
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_total_energy.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_total_energy.dat",ios::app);}
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
      ...
      coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/acceptance.dat",ios::app);
      if(_nattempts > 0) _fr_accept = double(_naccepted)/double(_nattempts);
      else _fr_accept = 0.0; 
      coutf << setw(12) << blk << setw(12) << _fr_accept << endl;
      coutf.close();
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
      ...
      if (_measure_magnet){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_magnet_"+oss_m.str()+"_.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_magnet_"+oss_m.str()+"_.dat",ios::app);}
      ...
      if (_measure_cv ){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_cv.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_cv.dat",ios::app);}
      ...
      if (_measure_chi ){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/METROPOLIS_chi.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/EQUILIBRATION/GIBBS_chi.dat",ios::app);}
...
///EVOLUTION CASE ////////////////////////////////////////////////////////////////////
    case MODE2:{
      // POTENTIAL ENERGY //////////////////////////////////////////////////////////
      if (_measure_penergy){
        if (_task == "autocorrelation"){coutf.open("../OUTPUT/"+_SMT+ "/AUTOCORRELATION/"+ _phase +"/potential_energy.dat",ios::app);
        }else{coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/potential_energy.dat",ios::app);}
      ...
      // KINETIC ENERGY ////////////////////////////////////////////////////////////
      if (_measure_kenergy){
        coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/kinetic_energy.dat",ios::app);
      ...
      if (_measure_tenergy){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_total_energy_" + oss.str() + "_.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_total_energy_" + oss.str() + "_.dat",ios::app);}
        }else{coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/total_energy.dat",ios::app);}
        average  = _average(_index_tenergy);
        sum_average = _global_av(_index_tenergy);
        sum_ave2 = _global_av2(_index_tenergy);
        coutf << setw(12) << blk
      ...
      if (_measure_temp){
        ...
        oss << fixed << std::setprecision(2) << _t_temporanea;
        string formatted_temp = oss.str();
        coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/temperature_" + formatted_temp +".dat",ios::app);
        average  = _average(_index_temp);
        sum_average = _global_av(_index_temp);
        sum_ave2 = _global_av2(_index_temp);
        coutf << setw(12) << blk
      ...
      if (_measure_pressure){
          coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/pressure.dat",ios::app);
      ...
      if (_measure_magnet){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_magnet_" + oss.str() + "_"+oss_m.str()+"_.dat",ios::app);}
        if (_sim_type == 3 ){
          if(_H==0.){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_magnet_" + oss.str() + "_.dat",ios::app);
          }else{coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_magnet_" + oss.str() + "_"+oss_m.str()+"_.dat",ios::app);}
        }
      ...
      if (_measure_cv){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_cv_" + oss.str() + "_.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_cv_" + oss.str() + "_.dat",ios::app);}
        average  = _average(_index_cv);
        sum_average = _global_av(_index_cv);
        sum_ave2 = _global_av2(_index_cv);
        coutf << setw(12) << blk
      ...
      if (_measure_chi){
        ...
        if (_sim_type == 2 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/METROPOLIS_chi_" + oss.str() + "_.dat",ios::app);}
        if (_sim_type == 3 ){coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/GIBBS_chi_" + oss.str() + "_.dat",ios::app);}
        average  = _average(_index_chi);
        sum_average = _global_av(_index_chi);
        sum_ave2 = _global_av2(_index_chi);
        coutf << setw(12) << blk
      ...
      coutf.open("../OUTPUT/"+_SMT+ "/"+ _phase +"/acceptance.dat",ios::app);
      if(_nattempts > 0) _fr_accept = double(_naccepted)/double(_nattempts);
      else _fr_accept = 0.0; 
      coutf << setw(12) << blk << setw(12) << _fr_accept << endl;
      coutf.close();
...
bool System :: equilibration_fine(double att_Temp,double  counter ){
      ...
      diff_T = fabs(ave_temp + err_temp - att_Temp);
      if ( diff_T  <= _T_precision ){ 
        double T_star = 0;
        k = false;
        cout_t << _temp << endl;
        cout_t.close();
        if (counter == 1. ){
          _best_diff = diff_T ;
          modify_temp_input("../INPUT/"+_method+"/"+ _phase +"/input.dat", _temp );
          }
        if ( counter != 1. && diff_T < _best_diff ){
          _best_diff = diff_T;
          modify_temp_input("../INPUT/"+_method+"/"+ _phase +"/input.dat", _temp );
        }
     ...
     if (_sim_type == 1){
      ...
      double delta_new= 0.;
        if (_fr_accept > 0.5 + 0.005){
          ...
          modify_delta_input("../INPUT/"+_method+"/"+ _phase +"/input_equilibration.dat", delta_new );
          k = false;
          ...
          modify_delta_input("../INPUT/"+_method+"/"+ _phase +"/input.dat", _delta );
          k = true;
        }
...
double System :: _set_T_attended(string phase){
  if (_phase == "GAS"){
    ...
    _T_attesa = 0.8;
    return _T_attesa;
  } else if (_phase == "ISING"){
    return _T_attesa = 2.0;
  }
...
void System :: modify_delta_input(const string& filename, double new_delta) {
    ifstream inputFile(filename);
    if (!inputFile.is_open()) {
        std::cerr << "Error: Could not open the file " << filename << std::endl;
        return;
    }
    ostringstream DeltaStream;
    string line;

    while (getline(inputFile, line)) {
        // Searching line named "DELTA"
        if (line.find("DELTA") != string::npos) {
            // Costruisci la nuova riga con T_STAR
            ostringstream newLine;
            newLine << "DELTA                   " << new_delta;
            DeltaStream << newLine.str() << endl;
        } else {
            // copy line without modifying
            DeltaStream << line << endl;
        }
    }
    ...
    outputFile << DeltaStream.str();
    outputFile.close();
}

void System:: set_SMT_of_system(){
  if (_sim_type == 1){
    _SMT = "MC_NVT";
  }else if(_sim_type == 0){
    _SMT = "MD_NVE";
  }else if (_sim_type == 2 || _sim_type == 3){
    _SMT = "";
  }
};

void System:: set_task(string task, string method){
  _task = task;
  _method= method;
}
/****************************************************************
